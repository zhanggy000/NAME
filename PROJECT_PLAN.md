...
| 4.5 | 实现 Step3 表单：评分权重滑块（默认/高级模式） | [x] | 2026-05-25 | Codex | 首页已支持高级权重滑块，后端 generate/score 支持自定义权重 |
| 4.6 | 实现结果页：Top 10 候选列表（卡片视图） | [x] | 2026-05-25 | Claude | |
| 4.7 | 实现详细评分卡组件（五维雷达图 + 拆解说明） | [x] | 2026-05-25 | Claude | 进度条版（雷达图待） |
| 4.8 | 实现"同字名人"展开面板 | [x] | 2026-05-25 | Codex | 候选评分卡展开后按字加载同字名人 |
| 4.9 | 实现"典籍出处"展开面板 | [x] | 2026-05-25 | Codex | 候选评分卡展开后按字加载典籍出处 |
| 4.10 | 实现历史记录页（localStorage 缓存） | [x] | 2026-05-25 | Codex | 生成后保存最近 20 条记录，新增 /history 查看与清空 |
| 4.11 | 实现导出 PDF 评分报告功能 | [x] | 2026-05-25 | Codex | 结果区新增导出 PDF 打印入口，并补充 print 样式 |
...
| 6.2 | 编写 API 层集成测试 | [x] | 2026-05-25 | Codex | tests/test_api.py 覆盖核心 API，并新增自定义权重用例 |
...
| 7.3 | 配置自定义域名 + HTTPS | [ ] | | | |
| 7.4 | 配置 Sentry 错误监控 | [x] | 2026-05-25 | Codex | 支持 SENTRY_DSN 可选启用，未配置时本地无影响 |
...
| 2026-05-25 | 新增五维评分自定义权重能力，并在首页表单接入高级权重滑块 | Codex | Phase 4 前端界面 |
| 2026-05-25 | 候选评分卡新增同字名人与典籍出处展开内容 | Codex | Phase 4 前端界面 |
| 2026-05-25 | 新增 localStorage 历史记录页，保存最近 20 次生成摘要 | Codex | Phase 4 前端界面 |
| 2026-05-25 | 结果页新增导出 PDF 打印入口与打印样式 | Codex | Phase 4 前端界面 |
| 2026-05-25 | 补正 API 集成测试状态，确认核心端点已有测试覆盖 | Codex | Phase 6 测试与优化 |
| 2026-05-25 | 新增候选生成性能测试，锁定 Top 10 生成低于 2 秒目标 | Codex | Phase 6 测试与优化 |
| 2026-05-25 | 补充 LLM 成本分析文档，记录缓存命中率和调用成本优化方向 | Codex | Phase 6 测试与优化 |
| 2026-05-25 | 增加复姓基础支持，生成和评分可处理欧阳等两字姓 | Codex | Phase 6 测试与优化 |
