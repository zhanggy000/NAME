...
| 阶段 | 任务数 | 已完成 | 进度 |
|---|---|---|---|
| Phase 0 · 项目初始化 | 6 | 5 | 83% |
| Phase 1 · 数据层 | 18 | 6 | 33% |
| Phase 2 · 算法核心 | 14 | 14 | 100% ✅ |
| Phase 3 · 后端 API | 10 | 7 | 70% |
| Phase 4 · 前端界面 | 12 | 0 | 0% |
...
| 0.5 | 创建 Node 项目骨架（`package.json`） | [ ] | | | 待 Phase 4 前端启动时建 |
...
| 1.2 | 收集康熙笔画数据源（开源 JSON / 汉典爬取） | [ ] | | | 种子已含 73 字手工录入；批量爬取待做 |
| 1.3 | 收集字五行数据源（多源对照：字源、字义、偏旁） | [ ] | | | 同上，种子已含 |
...
| 1.17 | 录入易经 64 卦基础信息（卦名、卦象、卦辞、关联五行） | [ ] | | | schema 已建，数据待录 |
| 1.18 | 录入谐音风险词库（普通话 + 主要方言） | [ ] | | | schema 已建，数据待录 |
